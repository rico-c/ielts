    ...
    <main className="ielts-shell min-h-screen px-4 py-6 sm:px-6 lg:px-10">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <header className="glass-panel flex items-center justify-between rounded-[1.5rem] px-5 py-4 sm:px-6">
          <Link href="/" className="font-display text-2xl italic text-slate-900">
            IELTS env.d.ts
          </Link>
          <div className="flex items-center gap-3">
            {userId ? (
              ...
              <Link href="/sign-in" className="accent-gradient rounded-full px-5 py-2 text-sm font-semibold text-white">
                登录
              </Link>
              </>
            )}
          ...
          <div className="grid gap-10 px-6 py-8 sm:px-8 sm:py-10 lg:grid-cols-[1.15fr_0.85fr] lg:px-12 lg:py-14">
            <div className="max-w-3xl">
              ...
              <div className="mt-10 grid gap-3 sm:grid-cols-2">
                {stats.map(([value, label]) => (
                  <div key={value} className="rounded-[1.4rem] border border-white/60 bg-white/56 px-4 py-4">
                    <div className="text-sm uppercase tracking-[0.22em] text-[var(--accent-deep)]">{value}</div>
            ...
            <div className="relative flex min-h-[420px] flex-col justify-between rounded-[2rem] border border-white/60 bg-[linear-gradient(180deg,rgba(255,255,255,0.72),rgba(245,232,220,0.92))] p-6 sm:p-8">
              ...
              <div className="relative">
                <div className="text-xs uppercase tracking-[0.24em] text-slate-500">Today&apos;s flow</div>
                <div className="mt-4 space-y-4">
                  {milestones.map((item, index) => (
                    <div key={item} className="flex gap-4 rounded-[1.25rem] border border-white/70 bg-white/72 p-4">
              ...
              <div className="relative mt-6 rounded-[1.5rem] bg-slate-900 px-5 py-5 text-white">
                <div className="text-xs uppercase tracking-[0.24em] text-white/60">Positioning</div>
                <p className="mt-3 text-sm leading-7 text-white/84">
                  不是单纯“放一个聊天框”，而是把首页做成雅思备考的总控台，既能承接流量，也能直接开始训练。
                </p>
                <p className="mt-3 text-sm leading-7 text-white/60">
                  登录后进入受保护的 dashboard，继续做题、看题库和使用个人训练入口。
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          {capabilityCards.map((card) => (
            <article key={card.title} className="glass-panel rounded-[1.75rem] p-6">
              <div className="text-xs uppercase tracking-[0.24em] text-[var(--accent-deep)]">{card.eyebrow}</div>
              <h2 className="mt-4 text-2xl font-semibold leading-8 text-slate-900">{card.title}</h2>
              <p className="text-ink-soft mt-4 text-sm leading-7">{card.body}</p>
            </article>
          ))}
        </section>
          ...
          <div className="glass-panel rounded-[2rem] p-6 sm:p-8">
            ...
            <div className="mt-8 grid gap-4">
              {moduleCards.map(([title, desc]) => (
                <div key={title} className="rounded-[1.4rem] border border-white/70 bg-white/60 p-5">
          ...
          <div className="glass-panel rounded-[2rem] p-6 sm:p-8">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              {[
                ...
                <div key={title} className="rounded-[1.4rem] border border-[var(--line)] bg-[var(--surface-strong)] p-5">
                  <div className="text-sm uppercase tracking-[0.2em] text-[var(--accent-deep)]">{title}</div>
        ...
        <section id="voice-lab" className="grid gap-6 lg:grid-cols-[0.78fr_1.22fr]">
          <div className="glass-panel rounded-[2rem] p-6 sm:p-8">
            <div className="section-kicker inline-flex rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em]">
              Voice assistant
            </div>
            ...
            <p className="text-ink-soft mt-5 text-sm leading-7">
              这里保留了现有 ElevenLabs 语音对话能力，但视觉上已经并入落地页。用户进入首页后，不需要理解技术细节，就能直接开始口语会话。
            </p>
          </div>
          <ElevenLabsVoiceAssistant agentId={agentId} />
        </section>

        <section id="test-preview" className="glass-panel rounded-[2rem] p-3 sm:p-4">
          <div className="px-4 pb-2 pt-3 sm:px-6">
            ...
            <p className="text-ink-soft mt-4 max-w-3xl text-sm leading-7">
              下面直接挂载最新 IELTS 试题数据组件，让落地页既承担转化，也承担产品预览。这样用户在点击注册前，就已经知道题型、数据结构和练习体验是什么。
            </p>
          </div>
          <IeltsTestRenderer />
        </section>
