...
// const CONFIG = {
//   pageUrl: 'https://practicepteonline.com/ielts-listening-test-203/',
//   outputPath: '', // e.g. './ielts-201.json'
//   writeDb: true,
//   dbName: 'ielts',
...
// function buildStructuredQuestions(questionText, answers) {
...
//     if (/^(Complete the|Choose\b|Write\b|Match\b)/i.test(line)) {
//       flushActiveQuestion();
//       flushSharedBlock();
...
//     const optionMatch = line.match(/^_+\s*([A-Z])\s+(.+)$/);
//     if (optionMatch) {
...
//   test_id, part_id, question_no, question_type, subtitle, instruction, prompt, answer, table_ref
// ) VALUES (
//   ${testIdExpr}, ${partIdExpr}, ${sqlInt(q.number)}, ${sqlQuote(q.type)}, ${sqlQuote(q.subtitle || '')},
//   ${sqlQuote(q.instruction || '')}, ${sqlQuote(q.prompt || '')}, ${sqlQuote(q.answer || '')}, ${sqlQuote(q.tableRef)}
// );`
//     );
...
//   result.questions = buildStructuredQuestions(questionText, result.answers);
